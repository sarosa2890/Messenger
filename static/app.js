...
const fav = chats.find(c=>c.peer.username==="Избранное"); 
if(fav){ openChat(fav); } 
else if(chats.length>0){ openChat(chats[0]); }
...
function appendMsg(m){
  const div = el("div","msg "+(m.sender===me.username?"me":"other"));
  if(m.type==="text"){ div.textContent = m.text; }
  if(m.type==="gif"){ const img = el("img","gif"); img.src=m.url; div.appendChild(img); }
  if(m.sender===me.username){
    const s = el("span","msg-status");
    s.textContent = statusIcon(m.status);
    div.appendChild(s);
  }
